...
type s6WaitCond struct {
	...
	// 指向 chanhkn 的指针，发信号用的。用于实现可以控制超时的 Wait()
...
func (p7this *s6WaitCond) f8WaitWithTimeout(i9ctx context.Context) error {
	...
	case <-i9ctx.Done():
		...
		return i9ctx.Err()
	case <-p7c7n:
		// 等到通知了，说明有 gorouting 调用了 Broadcast() 关闭了 chanhkn，把锁加回来。
...
func (p7this *s6WaitCond) f8Broadcast() {
	...
	// 用新的 chanhkn 替换旧的 chanhkn，注意要用原子操作。
	p7c7NOld := atomic.SwapPointer(&p7this.p7c7Notify, unsafe.Pointer(&p7c7NNew))
	// 关闭旧的 chanhkn，相当于做了一次广播，所有阻塞着的 gorouting 都会读到零值
