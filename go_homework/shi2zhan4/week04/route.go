...
func (r *router) findRoute(method string, path string) (*matchInfo, bool) {
	...
	for _, s := range segs {
		...
		if root.typ == nodeTypeAny {
			if root.children == nil && root.starChild == nil && root.paramChild == nil && root.regChild == nil {
				break
			}
		}
		if matchParam {
			if root.typ == nodeTypeParam {
				...
				if root.regExpr == nil {
					t4regIndex := strings.Index(root.path, "(")
					root.regExpr = regexp.MustCompile(root.path[t4regIndex+1 : len(root.path)-1])
				}
				s5res := root.regExpr.FindStringSubmatch(s)
				if nil != s5res {
					mi.addValue(root.paramName, s5res[0])
				} else {
					return nil, false
				}
			}
	...
	if path[0] == ':' {
		...
		if -1 != t4regIndex {
			...
			n.regChild = &node{
				typ:       nodeTypeReg,
				path:      path,
				paramName: path[1:t4regIndex],
				//regExpr:   regexp.MustCompile(path[t4regIndex : len(path)-1]),
			}
			return n.regChild
		} else {
			if n.starChild != nil {
