...
func main() {
  ...
  //所以dog是没有SetName方法的，不满足接口实现类型的定义。
  _, ok := interface{}(dog).(Pet)
  fmt.Printf("dog is Pet: %v\r\n", ok) //false
  _, ok = interface{}(&dog).(Pet)
  fmt.Printf("*dog is Pet: %v\r\n", ok) //true

  var pet Pet = &dog //这时pet的静态类型是Pet（接口），动态类型是*Dog（接口实现）
  fmt.Printf("pet is %s, name is %q\r\n", pet.Category(), pet.Name())

  var pet2 Pet2 = dog    //Pet2去掉了指针方法，所以这里pet2的赋值就变成了dog的副本
  dog.SetName("big dog") //这里修改了dog的值，但是副本不受影响，所以pet2的值不变
