...
import (
	middleware2 "demo-golang/framework/web/middleware"
	shutdown2 "demo-golang/framework/web/shutdown"
	"fmt"
	"net/http"
	"testing"
	"time"
...
func makeOpenService() *S6HTTPService {
	...
	p7s6hh.F8AddMiddleware(
		middleware2.F8RecoveryMiddleware(),
		middleware2.F8ReqBodyMiddleware(),
		middleware2.F8LogMiddleware(),
	)
	...
	p7s6hh.F8Get("/hello/world", f8handler, middleware2.F8TestMiddleware("/hello"), middleware2.F8TestMiddleware("/world"))
	p7s6hh.F8Get("/hello/*", f8handler, middleware2.F8TestMiddleware("/hello/*"))
	...
	p7s6hs.F8AddShutdownCallback(
		shutdown2.F8CacheShutdownCallback,
		shutdown2.F8CountShutdownCallback,
	)
...
func makeAdminService() *S6HTTPService {
	...
	p7s6hh.F8Group(
		"/admin",
		[]F8HTTPMiddlewareFunc{middleware2.F8TestMiddleware("admin")},
		[]S6RouteData{
