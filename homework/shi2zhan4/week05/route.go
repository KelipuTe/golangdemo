...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	return deepfindMdls(root, segs)
}

func deepfindMdls(root *node, segs []string) []Middleware {
	s5mid := make([]Middleware, 0)
	if nil != root.mdls {
		for _, item := range root.mdls {
			s5mid = append(s5mid, item)
		}
	}
	...
	cur := root
	t4starChild := cur.starChild

	var ok bool
	cur, _, ok = cur.childOf(segs[0])
	if !ok {
		return s5mid
	}

	if t4starChild != nil && t4starChild != cur {
		res := deepfindMdls(t4starChild, segs[1:])
