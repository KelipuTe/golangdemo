...
import (
	"context"
	"database/sql"
	"demo-golang/homework/shi2zhan4/week07/internal/errs"
	"demo-golang/homework/shi2zhan4/week07/model"
	"strings"
)
...
func (s *Selector[T]) Build() (*Query, error) {
	...
	if len(s.groupBy) > 0 {
		...
		for i, v := range s.groupBy {
			...
			if err = s.buildColumn(v.name, ""); err != nil {
				return nil, err
			}
		}
	}
	...
	if len(s.orderBy) > 0 {
		...
		for i, v := range s.orderBy {
			...
			if err = s.buildColumn(v.col, ""); err != nil {
				return nil, err
			}
			s.sb.WriteByte(' ')
			s.sb.WriteString(v.order)
		}
	}
